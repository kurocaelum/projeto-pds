...
jQuery(document).ready(function($){
    $('#form_ordem_servico').submit(function() {
        ...
        $.ajax({
                type: 'POST',
                dataType: 'json',
                url: 'http://pds.dev.anaju.me/business/controller/controleCadastroOrdemServicoMarcenaria.php',
                async: true,
                data: dados,
...
jQuery(document).ready(function($){
    $('#form_servico').submit(function() {
        ...
        $.ajax({
                type: 'POST',
                dataType: 'json',
                url: 'http://pds.dev.anaju.me/business/controller/controleCadastroServicoMarcenaria.php',
                async: true,
                data: dados,
            error: function(enviado) {
...
function resultCarregarServicos(listaServicos){
    ...
    for(var k in jsonLista) {
        ...
        tabelaServicos = tabelaServicos+'<td class="altura">'+jsonLista[k].altura+'</td>';
        tabelaServicos = tabelaServicos+'<td class="comprimento">'+jsonLista[k].comprimento+'</td>';
    ...
    $('#form_tipo_servico').submit(function() {
        dados = $('#form_tipo_servico').serialize();
            $.ajax({
                    type: 'POST',
                    dataType: 'json',
                    url: 'http://pds.dev.anaju.me/business/controller/controleCadastroTipoServicoMarcenaria.php',
                    async: true,
                    data: dados,
                error: function(enviado) {
...
function resultCarregarTipoServico(listaTiposServicos){
    ...
    for(var k in jsonLista) {
        if(jsonLista[k].tempo_extra_fixo != "-1"){
            ...
            tabelaTiposServicos = tabelaTiposServicos+'<td class="porcentagem_ferramenta_tipo_servico">'+jsonLista[k].porcentagem_ferramenta+'</td>';
    ...
    $('.editar_tipo_servico').on('click', function() {
        ...
        $("#form_input_tempo_extra_fixo").val( $("#tipo_servico"+id_editar).find(".porcentagem_ferramenta_tipo_servico").text() );
